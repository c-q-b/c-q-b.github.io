...
工程指标根据提供的简历整理。工程落地成果集中展示硬件设计、脑电采集、在线升级和低功耗管理；硬件与固件板块进一步介绍 ADS1299 菊花链采集板、STM32H743 数字处理板，以及采集驱动、数据解析与 IIR 滤波、USB 上传、多设备同步和日志调试。
...
公开图片已写入 CQB 水印；无水印备份保存在本地 `.preview/original-images/`，不会发布到 GitHub Pages。
